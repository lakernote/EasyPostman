...
HTTP 请求准备放在 `com.laker.postman.http.request.*`；具体运行时适配放在 `com.laker.postman.http.runtime.*`。这些包不能直接弹 Swing 对话框，也不能暴露 UI component 类型。UI 反馈应通过结构化结果返回，或委托给 UI 层 callback。
...
把 model 需要使用的 HTTP header 常量下沉到 foundation，或把 header normalize 逻辑移出 `HttpResponse`，放到 HTTP response mapper。`HttpResponse` 不能依赖 app service HTTP 包。
