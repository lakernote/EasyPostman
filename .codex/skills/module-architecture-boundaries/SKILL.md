...
- I18n mechanism belongs in `foundation`.
- I18n resources follow their owner: shared UI resources in `ui`, host resources in `app`, plugin resources in each plugin.
...
- Theme tokens, semantic colors, icon color strategies, and reusable UI resources belong in `ui`; FlatLaf installation and theme switching belong in `platform` once decoupled from app-specific wiring.
