...
public class PreparedRequestFactory {
    ...
    private static PreparedRequest buildEffectiveItem(HttpRequestItem effectiveItem) {
        ...
        req.followRedirects = HttpRequestSettingsResolver.resolveFollowRedirects(effectiveItem);
        req.cookieJarEnabled = HttpRequestSettingsResolver.resolveCookieJarEnabled(effectiveItem);
        req.sslVerificationEnabled = HttpRequestSettingsResolver.resolveSslVerificationEnabled(effectiveItem);
        req.httpVersion = HttpRequestSettingsResolver.resolveHttpVersion(effectiveItem);
        req.requestTimeoutMs = HttpRequestSettingsResolver.resolveRequestTimeoutMs(effectiveItem);
     ...
     * 由 {@link HttpUrlUtil#buildEncodedUrl} 统一完成去重 + 编码。
     */
     ...
     * 仍交给 PreparedRequestFinalizer 在发送前兜底补齐。
     */
