...
import com.laker.postman.plugin.market.PluginCatalogEntry;
import com.laker.postman.plugin.market.PluginCatalogService;
import com.laker.postman.plugin.market.PluginInstallerService;
...
public class PluginManagerDialog extends JDialog {
    ...
    private void loadSavedCatalogIfPresent() {
        String catalogUrl = PluginCatalogService.getCatalogUrl();
    ...
    private void reloadPlugins() {
        ...
        List<PluginFileInfo> plugins = PluginRuntime.getInstalledPlugins();
    ...
    private void toggleSelectedInstalledPlugin(boolean enabled) {
        ...
        PluginRuntime.setPluginEnabled(selected.descriptor().id(), enabled);
        reloadPlugins();
    }
    ...
    private void installLocalPluginJar() {
        ...
        try {
            PluginFileInfo installed = PluginInstallerService.installPluginJar(source);
    ...
    private void loadCatalog() {
        String catalogUrl = catalogUrlField.getText().trim();
    ...
    private void installSelectedCatalogPlugin() {
        ...
        new SwingWorker<PluginFileInfo, Void>() {
            ...
            protected PluginFileInfo doInBackground() throws Exception {
                return PluginInstallerService.installCatalogPlugin(selected);
            }
    ...
    private static String buildCompatibilityText(PluginDescriptor descriptor, boolean compatible) {
        ...
        String current = PluginRuntime.getCurrentAppVersion();
